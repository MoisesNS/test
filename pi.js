...
var lastCallback = "";
var mqttClientId = 'nutsoftPi_'+appID+"_"+idDevice;
var printLabelData="";
    ...
    getIP(function (err, externalIP) {
         console.log("getIp");
                ...
                deviceIp(null,function(netDevs){
                    data.network = netDevs;
                    deviceData = data;
                    mqttClient.publish(appID+'/update',JSON.stringify(data));
...
function radio(tpc,msg){
    ...
    var val = msg.val;
    var radio = msg.radio;
    var topic = tpc+"/radio/"+token;

    switch(op) {
		case "play":
            if(radio){
                cliCbPub("mpc play "+radio,topic);
            }
            else{
                cliCbPub("mpc play",topic);
            }
			break;
    	case "status" :
    	    cliCbPub("mpc",topic);
			break;
		case "playlist" :
		    cliCbPub("mpc load",topic);
			break;
		case "listUpdate" :
		    radioListUpdate(msg.list,topic);
			break;
		case "restart" :
		     cli("mpc load default",topic);
			break;
		case "pause" :
		    cliCbPub("mpc pause",topic);
			break;
		case "refresh" :
		    cli("mpc stop");
		    cliCbPub("mpc play",topic);
		...
		case "volumeDown" :
			...
			break;
		case "add" :
		    cliCbPub("mpc add "+url,topic);
			break;
		case "output" :
		    cliCbPub("sudo amixer -c 0 cset numid=3 "+val,topic);
        case "stop" :
            cliCbPub("mpc stop",topic);
        case "goTo" :
            cliCbPub("mpc stop",topic);
			break;
		case "next" :
            cliCbPub("mpc next",topic);
			break;
		case "prev" :
            cliCbPub("mpc prev",topic);
			break;
		case "reset" :
		    cliCb("sudo service mpd restart",function(res){
                console.log("play res2");
                console.log({res:res});
                cliCbPub("mpc",topic);
            });
...
function radioListUpdate(list,topic){
    var count = 0;
    console.log("radioListUpdate");
    cli("mpc stop");
    cliCb("mpc clear",function(res){
        async.each(list, function(s,cbLines){
            count ++;
            cliCb("mpc add "+s.url,function(res){
                console.log(JSON.stringify(s));
                console.log(JSON.stringify(s.url));
                console.log("upd res "+count);
                console.log({res:res});
                 cbLines();
            });
        },function(err){
            cli("mpc save default");
            cliCb("mpc play",function(res){
                console.log("play res2");
                console.log({res:res});
                cliCbPub("mpc",topic);
...
function cliCbPub(cli,topic){
    cliCb(cli,function(res){
        mqttClient.publish(topic,JSON.stringify(res));
    });
}
