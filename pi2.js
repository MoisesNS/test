...
var labelPrinterHost = '192.168.33.7';
...
var topicList = {
        '0c89c0e5-9bb9-11e5-bb98-42010af00002/1/1/checkDevices' : 1,
    };
...
//        		printProductLabelRaster(topicStr,data);
   ...
   //     		printLabelShipping(topicStr,data);
    			break;
    		case "checkDevices":
                checkDevices();
        	    break;
...
console.log("printData")
	...
	if(serial==1){
	    ...
	    client.connect("6001", "192.168.0.253", function() {
...
function insert_zero_init(string, leng){
...
//    printLabelData += '^'+'L'+'S'+'0'+'1'+'0';
  ...
  //  printLabelData += page_length(168); //1mm->12dots
   ...
   select_and_insert('barcode_text', data.code);
   select_and_insert('tax_text',"IVA 23%");
   select_and_insert('price',data.price);
   select_and_insert('product_name',data.name);
   select_and_insert('code', Number(data.code));
...
//printLabelData +=String.fromCharCode(27)+"iX32"+String.fromCharCode(2)+String.fromCharCode(0);
   var qt = insert_zero_init(data.qt,3);
   printLabelData += '^'+'C'+'N'+qt;
//printLabelData += '2^'+'C'+'R'+'2';
    ...
    var client = new net.Socket();
    console.log(data.device.mac);
    deviceIp(data.device.mac,function(labelPrinterHost){
    	console.log({labelPrinterHost:labelPrinterHost});
...
function printLabelShipping(topicStr,data){
	console.log({topicStr:topicStr,data:data});
    console.log("printLabelProduct");
...
//    printLabelData += '^'+'L'+'S'+'0'+'1'+'0';
 //   printLabelData += '^'+'L'+'S'+'0'+'0'+'1';
  ...
  //  printLabelData += page_length(168); //1mm->12dots
   ...
   // printLabelData += template_operation("cut");
    select_and_insert('shipping',"André Vicente");
    ...
    switch(op) {
    	...
    	case "status" :
            cliCb("mpc",function(res){
                ...
                console.log({msg:msg});
                console.log("Topic\n"+topic+"/"+msg.token);
                publish(topic+"/"+msg.token,JSON.stringify(msg));
            });
			break;
		case "playlist" :
